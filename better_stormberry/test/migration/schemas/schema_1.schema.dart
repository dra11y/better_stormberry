...
extension Schema1Repositories on PgDatabase {
...
class _BookRepository extends BaseRepository<PgDatabase>
  ...
  Future<List<int>> insert(List<BookInsertRequest> requests) async {
    ...
    var rows = await db.query(
      'INSERT INTO "books" ( "title", "is_best_selling", "author_id" )\n'
      'VALUES ${requests.map((r) => '( ${values.add(r.title)}:text, ${values.add(r.isBestSelling)}:boolean, ${values.add(r.authorId)}:text )').join(', ')}\n'
      'RETURNING "id"',
      values.values,
    );
  ...
  Future<void> update(List<BookUpdateRequest> requests) async {
    ...
    await db.query(
      'UPDATE "books"\n'
      'SET "title" = COALESCE(UPDATED."title", "books"."title"), "is_best_selling" = COALESCE(UPDATED."is_best_selling", "books"."is_best_selling"), "author_id" = COALESCE(UPDATED."author_id", "books"."author_id")\n'
      'FROM ( VALUES ${requests.map((r) => '( ${values.add(r.id)}:int8::int8, ${values.add(r.title)}:text::text, ${values.add(r.isBestSelling)}:boolean::boolean, ${values.add(r.authorId)}:text::text )').join(', ')} )\n'
      'AS UPDATED("id", "title", "is_best_selling", "author_id")\n'
